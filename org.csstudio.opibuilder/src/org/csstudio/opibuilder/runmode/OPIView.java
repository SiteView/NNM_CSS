...
//					input.getToolTipText());
	...
	public void dispose() {
		super.dispose();
		LinkedHashMap<StringProperty, PVValueProperty> as = getDisplayModel().getPVMap();
		for(Entry<StringProperty, PVValueProperty> a : as.entrySet()){
			a.getKey().removeAllPropertyChangeListeners();
			System.out.println(new String(a.getKey().getDescription().getBytes()));
			System.out.println(new String(a.getValue().getDescription().getBytes()));
